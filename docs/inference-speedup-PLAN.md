...
- **Stage 3/4 dataset bug found + additively fixed (2026-07-02 ~16:08 PM):** the driver's stage 3/4 would have silently re-run scaling8 instead of bad_afdb26 (see SUMMARY DON'T for root cause). Cannot safely patch the live driver in place (bash already parsed its function body). Fix: a second, separate script `a6000_bad_afdb26_manual_driver.sh` launched to use GPU0/GPU2 (idle at the time, waiting on GPU1's 8XHT_A) immediately on the CORRECT bad_afdb26.csv, deferring GPU1 until the live driver's lock PID fully exits. The live driver's own (buggy) stage 3/4 will still fire later and harmlessly no-op (skip_existing on the already-complete scaling8 set) — no collision, since raw output is keyed by (cfg, protein_id) not by launching script. Once both nfe40+nfe50 finish on all 3 shards, re-run `campaign_oracle_summary.py` for the real final report (the live driver's own auto-generated `oracle_summary.txt` will be incomplete — use `oracle_summary_full.txt` from the manual driver instead).
- bad_afdb26 contains an outlier (8AUC_B, L=492, far larger than scaling8's max of 287) — projected ~11-14h alone based on observed L²·log(L) cost scaling. This will likely be the long pole for both the live driver's harmless no-op AND (for real) the manual driver's nfe40/nfe50 stages; let it run to natural completion rather than capping it, since a partial cutoff produces no oracle result for that protein either way.
- **Status as of 2026-07-02 6:16 PM:** live driver `a6000_hybrid_campaign_driver.sh` fully finished (`CAMPAIGN_DONE` 5:50 PM) — all 4 stages ran, its own no-op stage 3 hit a real (inconsequential) failure (see SUMMARY DON'T: GPU0 OOM from contention with the manual driver), stage 4 succeeded; its own `oracle_summary.txt` is broken (see SUMMARY DON'T: no venv activation) — ignore it, use the manual driver's output instead. Manual driver `a6000_bad_afdb26_manual_driver.sh` correctly detected the live driver's exit and launched GPU1's shard on schedule. Current: GPU0 still on 8AUC_B (started 4:08 PM), GPU1 on shard 1 (started 5:51 PM, now on 8TVL_A L=338, also sizeable), GPU2 progressing normally through its shard. `oracle_summary_full.txt` not yet written — won't appear until ALL nfe40 shards finish (gated by 8AUC_B) and then all nfe50 shards too.
- **Autonomous directive (user, 2026-07-03 1:57 AM): keep pushing/monitoring this campaign to completion for ~12h, until ~1:57 PM EDT 2026-07-03.** Status at directive time: 8AUC_B finished nfe40 in 8.36h (00:30 AM, faster than the 11-14h worst case) — GPU0 shard now on the remaining 4 small proteins (6UF2_A/7F7N_A/7MQQ_A/7ZK0_A), GPU1 shard on its 5th/last protein (7E7T_B), GPU2 shard on its 6th of 7 (7VZM_A, then 8F3K_A/8AP5_A left). nfe40 stage likely finishes within a few hours; nfe50 then re-samples all 26 proteins fresh (no skip_existing carryover across configs) — 8AUC_B at nfe50 now projects ~10.5h (scaled from the actual 8.36h nfe40 time), so the full nfe40+nfe50 campaign may not complete within the 12h window. Keep monitoring hourly via ScheduleWakeup (`create_trigger`/`send_later` still 404ing) regardless of whether it finishes in time — report real progress, don't force/cut anything short.
- **nfe40 stage fully complete (2026-07-03 4:33 AM, `NFE40_ALL_SHARDS_DONE`), took 12h25min total (4:08 PM->4:33 AM).** One graceful CUDA OOM recovery during GPU0's 8AUC_B scoring (auto-retried at batch_size=8 instead of 16, succeeded) — not a failure. nfe50 auto-started on all 3 GPUs at 4:33 AM with the SAME shard assignment (LPT bin-packing is deterministic): GPU0=8AUC_B again, GPU1=8TVL_A again, GPU2=7DME_A first. Projected 8AUC_B-at-nfe50 finish ~3:00 PM (scaling its actual 8.36h nfe40 time by 50/40) — past the 1:57 PM deadline by about an hour; continuing to push per the directive regardless.
- **Sanity check performed (2026-07-03, ~2:15 AM), see finding #12 in §3:** verified pipeline/data integrity (correct sample counts, no NaN, no missing files) and found real (non-bug) chain-break degradation for large/OOD proteins in the raw nfe40 samples.
