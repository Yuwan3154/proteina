...
from proteinfoundation.openfold_stub.utils import feats
...
def compute_fape(
    ...
    rand_clamp: Optional[bool] = False,  # Added by tg
    eps=1e-8,
) -> torch.Tensor:
    """
        Computes FAPE loss.
        ...
        Args:
            ...
            rand_clamp:
                Apply random clamping inside batch (if l1_clamp_distance is not None)
            eps:
    ...
    # Modified to random clamping within batch - not in original of code
    if l1_clamp_distance is not None:
        if not rand_clamp:
            error_dist = torch.clamp(error_dist, min=0, max=l1_clamp_distance)
        else:
            clamp_low = torch.zeros(error_dist.shape, device=error_dist.device)
            clamp_high = torch.where(
                torch.rand(error_dist.shape, device=error_dist.device) < 0.9,
                torch.ones(error_dist.shape, device=error_dist.device) * l1_clamp_distance,  # clamp with prob 0.9
                torch.ones(error_dist.shape, device=error_dist.device) * torch.inf,  # do not clamp with prob 0.1
            )
            error_dist = torch.clamp(error_dist, min=clamp_low, max=clamp_high)
