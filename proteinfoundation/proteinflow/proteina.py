...
class Proteina(ModelTrainerBase):
    def __init__(self, cfg_exp, store_dir=None):
        ...
        initial_modality = "contact_map" if self.contact_map_mode else "coordinates"
        self.fm = FlowMatcher(
    ...
    def extract_clean_sample(self, batch):
        ...
        """
        logger.info("DEBUG_TRACE: extract_clean_sample start")
        coords = batch["coords"]  # [b, n, atoms, 3]
        mask = batch["mask_dict"]["coords"][..., 0, 0]  # [b, n] boolean
        logger.info(f"DEBUG_TRACE: coords shape={coords.shape}, mask shape={mask.shape}")
        ...
        logger.info("DEBUG_TRACE: extract_clean_sample end")
        return (
            ang_to_nm(x_1),
            mask,
            batch_shape,
    ...
    def compute_contact_map_loss(
        ...
        npairs = torch.sum(pair_mask, dim=(-1, -2)) - torch.sum(mask, dim=-1) # [*]
    ...
    def compute_auxiliary_loss(
        ...
        if pred_pair_dists is not None:
            ...
            dist_mat_loss = dist_mat_loss / torch.clamp(den, min=1.0)  # guard tiny/zero
        else:
        ...
        if num_dist_buckets and pair_logits is not None:
            ...
            min_dist = self.cfg_exp.loss.get("distogram_min_bin", 0.0)
            max_dist = self.cfg_exp.loss.get(
                "distogram_max_bin",
