...
# ckpt every 2 steps, wandb offline. Phase 1: fresh -> step 6 (pretrain surgery load, sanity val, accum boundary, val,
...
# wandb stays ON as in production (log_wandb=False swaps in a TensorBoard logger the trainer's .log() calls cannot use),
# but offline, so the toy never reaches the project
...
TOY="opt.accumulate_grad_batches=2 opt.val_check_interval_optim_steps=2 opt.limit_val_batches=2 opt.warmup_steps=2
     log.checkpoint_every_n_steps=2 log.last_ckpt_every_n_steps=2
