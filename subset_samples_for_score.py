        ...
        for i in range(bin_num):
            if counts[i] > 0:
                ...
                for in_file, out_file in zip(in_files, out_files):
                    result = subprocess.run(["convert_cg2all", "-p", str(inference_path / in_file), "-o", str(inference_path / out_file), "--cg", "ca", "--device", "cpu"], capture_output=True, text=True, check=True)
